...
# Stepwise with R0 fixed implying variable Beta
...
vswsim <- data.frame(generation = numeric(),
                     simnumb = character()
)
# Note: simulation isolated in sims.R
varistepwise <- function(iterations,vectortype,cohortsize,transmissionrate,recoveryrate){
  for (k in 1:iterations){
    ...
    for (i in 1:length(vector)){
      ...
      contacts = rpois(1,transmissionrate/(recoveryrate*tau)) # generate number of secondary contacts
    ...
    if (length(gidata) == 0){
      ...
      vswsim <- rbind(vswsim,namedintervals)}
  }
  names(vswsim) <- c('generation', 'simnumb') # insert names in multisim
  vswsim <- vswsim[!is.na(vswsim$generation),] #remove NA from no contact or other
  vswsim <- vswsim %>%
  ...
  vswsim$count <- ave(vswsim$generation,vswsim$simnumb, FUN=seq_along) # each time receives number of infection it was by sim
  ...
  p1 <- vswsim %>%
    ...
    ggtitle(paste0("FswPDF FGIs with Beta = variable, Gamma = ", recoveryrate, "initcohort type:", vectortype, ".")) +
    facet_wrap(~simnumb)
  ...
  p2 <- vswsim %>%
    ...
    ggtitle(paste0("FswCDF FGIs with Beta = variable, Gamma = ", recoveryrate, "initcohort type:", vectortype, ".")) +
    facet_wrap(~simnumb)
