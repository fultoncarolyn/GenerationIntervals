# Forward Generation Interval Simulations with Delta Individual Infectiousness Profile
...
deltasim <- data.frame(generation = numeric(),
                       simnumb = character()
)
# Note: simulation isolated in sims.R
deltaprofile <- function(iterations,vectortype,cohortsize,transmissionrate,recoveryrate){
  for (k in 1:iterations){
      ...
      if (contacts == 0){
        ...
        for (j in 1:contacts){
          ...
          if (transmission == 0){
            ...
            # infection defined for each individual to concentrate infectiousness
    ...
    if (length(gidata) == 0){
      ...
      deltasim <- rbind(deltasim,namedintervals)}
  }
  names(deltasim) <- c('generation', 'simnumb') # insert names in multisim
  deltasim <- deltasim[!is.na(deltasim$generation),] #remove NA from no contact or other
  deltasim <- deltasim %>%
  ...
  deltasim$count <- ave(deltasim$generation,deltasim$simnumb, FUN=seq_along) # each time receives number of infection it was by sim
  ...
  maxgen = max(deltasim$generation)
